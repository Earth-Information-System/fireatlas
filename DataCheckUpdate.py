...
def check_VNP14IMGTDL_avail(year, month, day):
    ...
    fnmFC = os.path.join(dirFC,'SUOMI_VIIRS_C2_Global_VNP14IMGTDL_NRT_'+t.strftime('%Y%j')+'.txt')
    ...
    if os.path.exists(fnmFC):
        ...
        fnms = glob(dirFC+'SUOMI_VIIRS_C2_Global_VNP14IMGTDL_NRT_'+t.strftime('%Y')+'*.txt')
        if len(fnms) > 0:
            days = max([int(os.path.basename(fnm)[42:45]) for fnm in fnms])
            tmax = date(t.year,1,1) + timedelta(days=days-1)
            print('The latest VNP14IMGTDL available date is '+tmax.strftime('%Y%m%d')
                   +'(doy = '+tmax.strftime('%j')+')')
...
def update_VNP14IMGTDL(local_dir=None):
    ...
    for d in pd.date_range(dstart,today):
        ...
        strcmd = ycpy.util.wget(url=urlfnm,locdir=local_dir,robots_off=True,no_wget=False,timestamping=True,header='NASA')
...
def update_VJ114IMGTDL(local_dir=None):
    ...
    # Derive the date periods needed to download
    today = date.today()
    fnms = glob(local_dir+'NOAA20_VIIRS_C2_Global_VJ114IMGTDL_NRT_'+str(today.year)+'*.txt')
    if len(fnms)==0:
        ndays = 0
    else:
        doys = [int(d[-7:-4]) for d in fnms]
    ...
    for d in pd.date_range(dstart,today):
        urlfnm = urldir + "NOAA20_VIIRS_C2_Global_VJ114IMGTDL_NRT_"+d.strftime('%Y%j')+".txt"
        strcmd = ycpy.util.wget(url=urlfnm,locdir=local_dir,robots_off=True,no_wget=False,timestamping=True,header='NASA')
