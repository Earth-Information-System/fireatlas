...
for year in range(2012,2022):
    ...
    fta = gpd.read_file(pdir+'AK/'+str(year)+'/Summary/final'+str(year)+'.gpkg').to_crs(ref.crs)
    mcd = gpd.read_file(pdir+'AK/'+str(year)+'/Summary/final_mcd64'+str(year)+'.gpkg').to_crs(ref.crs)
    lakes = gpd.read_file(pdir+'AK/'+str(year)+'/Summary/final_lakes'+str(year)+'.gpkg')
...
df = pd.DataFrame(out, columns = ['year','ref_no','viirs_no','mcd64_no','lakes_no','ref_size','viirs_size','mcd64_size','lakes_size'])
df = df.set_index('year')
df_no = df.filter(like='no',axis=1)
df_size = df.filter(like='size',axis=1)
...
ign = gpd.read_file(pdir + '/2015_AK_'+str(alpha)+'/Summary/ign2015.gpkg').to_crs(ref.crs)
...
for year in range(2012,2022):
    ...
    fta = gpd.read_file(pdir+'NT/'+str(year)+'/Summary/final'+str(year)+'.gpkg').to_crs(ref.crs)
