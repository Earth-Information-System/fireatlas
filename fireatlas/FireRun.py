...
def CreekRegionSamplerun():
    ...
    logger.info(
        f"{(tend-tstart)/60.} minutes used for CreekRegionSamplerun with dask.")
...
def ChileSampleRun():
    ...
    logger.info(
        f"{(tend-tstart)/60.} minutes used for ChileSampleRun with dask.")
...
def BorealNA():
    ...
    logger.info(
        f"{(tend-tstart)/60.} minutes used for BorealNA for {tst} to {ted}.")
...
def ItalyGreeceNRT():
    ...
    logger.info(
        f"{(tend-tstart)/60.} minutes used for Italy and Greece for {tst} to {ted}.")
...
def QuebecSampleRun():
    ...
    logger.info(
        f"{(tend-tstart)/60.} minutes used for QuebecSampleRun with dask.")
...
def CArun32610():
    ...
    # temporarily set to 'pm' to avoid reading from previous year's data
    tst = (2012, 1, 20, "AM")
    # tst = (2012, 1, 1, "AM")
...
#     calculate and save single fire files
    FireGpkg_sfs.save_sfts_trng(tst, ted, regnm=region[0], layers=[
                                "perimeter", "fireline", "newfirepix", "nfplist"])
...
def WesternUSrunNRT():
    ...
    # ted = [2022,1,10,'AM']
    print(
        f"Running code from {tst} to {ted} with source {settings.FIRE_SOURCE}")
...
def constrainByShape_Run(perimeter_gdf_path, tst=None, ted=None, sat='SNPP', data_source='FRAP', id_col='INC_NUM'):
    ...
    for t in list_of_ts:
        preprocess.preprocess_region_t(
            t, region=region, read_region_location='local')

    # finally run fire_forward
    allfires, allpixels, t_saved = FireMain.Fire_Forward(
        tst=tst, ted=ted, restart=False, region=region, read_location='local', read_saved_location='local')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="registered MAAP.DPS jobs call ./run_dps.sh which delegates to this run function")
    parser.add_argument("run_function_name",
                        help="The name of the function in ./FireRun.py to call")
