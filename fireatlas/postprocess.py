...
def get_t_of_last_allfires_run(location: Literal["local", "s3"]):
    ...
    t: TimeStep
       latest t for which there are allfires and allpixels files
    
    """
    all_dir = postprocess.all_dir(tst, region, location=location)
    last_allfires = sorted(glob.glob(os.path.join(all_dir, "allfires*")))[-1]
    last_allpixels = sorted(glob.glob(os.path.join(all_dir, "allpixels*")))[-1]

    dt = pd.to_datetime(sorted([last_allfires, last_allpixels])[0].split("/")[-1].split("_")[1])
    return FireTime.dt2t(dt)
