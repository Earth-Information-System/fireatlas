...
Second, NRT data are also available via [OGC API](https://firenrt.delta-backend.com/). An [example notebook](https://docs.openveda.cloud/user-guide/notebooks/tutorials/mapping-fires.html) that demonstrates how to subset, analyze, and visualize the data outside of FIRMS is available on NASA’s VEDA platform.
...
For all fire events, regardless of fire size or duration, the FEDS algorithm provides a "snapshot" of the modeled fire perimeter every 12 hours. For fires that ultimately grow larger than 4 km2, these modeled perimeters and associated attributes are stored in sequence to track the growth and behavior of each large fire event.
