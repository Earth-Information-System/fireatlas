...
Twice a day, at 15:25 and 23:25 UTC, each scheduled region is run through the main FireForward algorithm to incorporate any new observations. The outputs from these runs are saved to `FEDSoutputs-v3/` and copied to the VEDA S3 bucket. This triggers a workflow that makes the outputs available on the [VEDA Features API](https://github.com/NASA-IMPACT/veda-features-api-cdk).
...
In a container based on the vanilla MAAP image (so that the `maap` Python package is available), run [`submit-dps-jobs.py`](https://github.com/Earth-Information-System/fireatlas/blob/main/maap_runtime/submit-dps-job.py). This calls the MAAP API and submits a job with the passed parameters.
...
Runs the [`alert-on-failed-dps-jobs`](https://github.com/Earth-Information-System/fireatlas/blob/main/.github/workflows/schedule-alert-failed-dps-jobs.yaml) action every hour. If a failure is detected, sends a notification to the `#fireatlas-alerts` channel in our Slack workspace. (This is neccessary because only the GitHub account that initiated the workflow will recieve an email notification if it fails.)
...
Uses `pytest` to automatically run test suite on any PR or pull to the main `main` branch. These include several integration tests that only run with the `--runslow` flag. These tests are slower and can take \~15 minutes to run.
