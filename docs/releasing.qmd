...
# Background 
To improve reproducibility and reliability, we use tagged releases in our GitHub repo to determine which version of the FEDS code our [NRT production system](nrt.qmd) uses. The NRT system does ***not*** automatically incorporate changes pushed to the main branch of the GitHub repository. For your changes to be reflected in the production system, you need to release a new version as described in this document. This will trigger DPS to build a new image with the tagged version of the code downloaded from the repository and installed. DPS will use this new image for all subsequent scheduled jobs, until another version is released.  

:::::: {.callout-note}
Releasing a new tagged version is not the only way to create a new DPS image! If you want to test changes on the DPS or run large jobs with an experimental branch, you can manually create and use your own custom DPS image as detailed in our [DPS documentation](custom_regions.qmd#running-different-feds-versions-on-dps). 
:::
...
* build and publishing `fireatlas` on PyPI 
  - NOTE: this step is currently paused; we are not publishing the package to PyPI at the moment
