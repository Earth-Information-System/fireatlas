...
def correct_nested_ids(mergetuple):
    ...
    if len(not_unique)>0:
        for key in not_unique:
            tgt1 = min([tgt[ind] for ind in range(len(tgt)) if src[ind] == key])  # tgt with smallest fid
...
def Fire_expand_rtree(allfires,afp,fids_ea,log=True):
    ...
    for ic in range(max(cid)+1):
        ...
        id_cfs = FireClustering.idx_intersection(ea_idx, cluster.b_box) # potential neighbours using spatial index
        clusterdone = False
        for id_cf in id_cfs:  # loop over all potential eafires
            if clusterdone == False:  # one cluster can only be appended to one existing object
                if eafirerngs[id_cf].intersects(hull): # determine if cluster touch fire connecting range
        ...
        if not expand_only:  # ignore creating new fires if expand_only is set to True
            if clusterdone is False:  # if the cluster is not added to existing active fires
