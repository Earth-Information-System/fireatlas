...
"""
def Yearbatchrun(year,region='',tst=None,ted=None,restart=False):
    ...
    FireGdf_merge.save_gdf_trng(tst=tst,ted=ted,fperim=True,fline=True,NFP_txt=True,region=region)
    ...
    FireGdf_ign.save_gdf_trng(tst,ted,region=region)
    FireGdf_final.save_gdf_trng(tst,ted,region=region)
    t31 = time.time()
    ...
    FireSummary.save_sum_1d(tst,ted,region)
