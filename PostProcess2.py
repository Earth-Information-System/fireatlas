...
def lake_stats(geom_orig, geom_lakes, counter=[],rtree=False):
    '''compute stats fields for lakes
    '''
    # import pyproj
    # geod = pyproj.Geod(ellps='WGS84') # geoid used for computing distances in m (if EPSG:4326)
    lake_border_tot = 0     # length of perimeter bordering lake (including inner lakes)
    lake_border_outer = 0   # length of perimeter bordering lake (only of thsi time step)
    lake_area = 0           # total lake area within the fire
    lake_no = 0             # number of lakes in fire perimeter
    
    # additional attributes
    if rtree:
    else:
        intsct = geom_lakes.difference(geom_orig) # linestring of intersection
    ...
    for i in range(len(intsct)):
        lake = intsct[i]
        if lake.within(geom_orig): # if lakes are within fire scar
            #temp = geod.geometry_area_perimeter(lake)
            #lake_area += abs(temp[0])
            # intsct_length += temp[1]
            lake_no += 1
            lake_area = lake.area
            lake_border_tot += lake.length
        else: # if lakes are bordering fire scar
            true_intsct = lake.intersection(geom_lakes)
            lake_border_tot += true_intsct.length
            lake_border_outer += true_intsct.length
            # intsct_length += geod.geometry_area_perimeter(true_intsct)[1]
    
    return lake_area, lake_no, lake_border_tot, lake_border_outer
...
def clip_lakes_1fire_outer(gdf_1d, lakediss, lake_idx):
    ...
    return geom, lake_area, intsct_length

def clip_lakes_1fire(geom, lakediss, outer=False, stats=True, rtree=False):
    ...
    lakediss : geopandas dataframe
        ...
        Polygon or MultiPolygon
    stats: bool,
        return stats (lake area & lake perimeter intersection legth) or only geometry
    ...
    if rtree: # we build and rtree index of lakes and use the intersection for loop
        import FireClustering
        lake_idx = FireClustering.build_rtree(lakediss) # build index of lakes
        # extract potential lakes in geometry bounding box
        counter = FireClustering.idx_intersection(lake_idx, geom.bounds)
    if not outer:
        import copy
        geom_orig = copy.deepcopy(geom) # geometry without lakes
        geom = geom.difference(lakediss.loc[1]['geometry']) # clip lakes
    
    if stats:
        lake_area,lake_border = lake_stats(geom_orig, geom, rtree, counter)
        out = (geom, lake_area, lake_border)
    else:
        out = geom
...
#         else:
#             if len(lakediss) > 0:
#                 clipped_geoms,lake_area,intsct_length = clip_lakes_1fire(geom, lakediss)
