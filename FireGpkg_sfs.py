...
def find_largefires(allfires,falim=4):
    ...
    '''
    large_ids = [fid for fid in (allfires.fids_active + allfires.fids_sleeper)
...
# gdf_sf.loc[t_m,'geometry'] = gdf_sf.loc[t_m,'geometry'].union(gs_t_m['geometry'])
    ...
    if 'n_pixels' in sfkeys: gdf_diss.loc[fid,'n_pixels'] = sum(gdf_1d.n_pixels)
    if 'n_newpixels' in sfkeys: gdf_diss.loc[fid,'n_newpixels'] = sum(gdf_1d.n_newpixels)
    if 'farea' in sfkeys: gdf_diss.loc[fid,'farea'] = sum(gdf_1d.farea)
    if 'fperim' in sfkeys: gdf_diss.loc[fid,'fperim'] = sum(gdf_1d.fperim)
    if 'flinelen' in sfkeys: gdf_diss.loc[fid,'flinelen'] = sum(gdf_1d.flinelen)
    if 't' in sfkeys: gdf_diss.loc[fid,'t'] = FireTime.t2dt(t)

    # weighted average computed for averages
    if ('pixden' in sfkeys) & ('farea' in sfkeys):
        gdf_1d = gdf_1d.assign(pixweigh = gdf_1d['pixden']*gdf_1d['farea']) ## IS THIS CORRECT?
...
def make_sf(t,regnm,layer,fids_m,fid,sfkeys):
    ...
    if layer != 'nfplist':
        ...
        gdf = FireIO.load_gpkgobj(t,regnm,layer=layer)   # read daily gdf: active and sleeper only
        ...
        gdf_1d = gdf_1d[sfkeys+['geometry']]   # drop some columns not used in single fire gpkg
...
def save_sfts_1f(fid,regnm,allfires=None,t=None,
